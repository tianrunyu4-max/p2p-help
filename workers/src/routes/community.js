...
▌激活金额：200元（全部P2P点对点直接打款给真实用户，平台不经手）
  - 见点奖 80元 → 直接打给邀请你的老板
  - 帮扶奖 30×2=60元 → 直接打给老板的2位已出局直推
  - 平级奖 5×12=60元 → 按邀请链向上追溯12层，每层5元
  - 合计：80+60+60=200元 ✓
...
export async function handleAI(request, env, pathname) {
  ...
  if (botMatch && request.method === 'POST') {
    ...
    const botPrompts = {
      deepsook: `你是"大龙虾 DeepSook"，1+1互助平台专属顾问，性格幽默接地气，结尾必须加🦞。\n\n最新制度：激活200元（见点80+帮扶30×2+平级5×12层），复投阈值900元，全程P2P直接打款。参与步骤：①填邀请码②系统匹配③逐笔付款截图④收款方确认→激活成功。老板位永久收见点奖80元，累计收款满900元需复投继续循环。\n\n回答简洁有趣，不确定的引导看平台说明。`,
